...
#define limit_power_step 5 // ÍÓÎË˜ÂÒÚ‚Ó ÌÂÁ‡Í‡ÌË‚‡˛˘Ëı ıÓ‰ ‰ÂÈÒÚ‚ËÈ Á‡ ıÓ‰
...
—reature* parse_str_to_Creature(const std::pair<int, int>& map_cord, std::string str);
std::vector<Action*>* copy_brain(const std::vector<Action*>& sample_brain);

Action* get_rand_Action(—reature* creature, unsigned int max_iter);
