	...
	if (last_draw_peep_Creature) {
		...
		if (peep_Сreature != nullptr) {
			SetWindowTextA(StaticPeepData, LPCSTR((
				"Энергия: " + std::to_string(peep_Сreature->get_energy())
				+ "  Энергия деления: " + std::to_string(peep_Сreature->get_lim_energy())
				+ "  Возраст: " + std::to_string(peep_Сreature->get_age())
				+ "  Действие: " + std::to_string(peep_Сreature->get_iter())
				+ "  Направление: " + string_by_dir_for_turn(peep_Сreature->get_dir())
...
void BuildObject() {
	...
	//std::vector<Creature_Scavenger::Сondition> br;
	//br.push_back(Creature_Scavenger::Сondition(
	//	1,
	//	-1, -1,
	//	nullptr,
	...
	//std::vector<Creature_Herbivore::Сondition> br;
	//br.push_back(Creature_Herbivore::Сondition(
	//	1,
	//	-1, -1,
	//	nullptr,
	//	Direction::UP, Type_Creature::Void, 1000,
