...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)  // обработка событий
{
	...
	case WM_COMMAND:
	{
		...
		{
			SetFocus(hWnd);
			if (peep_Сreature == nullptr) { MessageBoxA(hWnd, "Существо не выбрано.", "Внимание!", MB_OK | MB_ICONEXCLAMATION); break; }
			...
			if (GetSaveFileNameA(&ofn)) {
				if (CheckExpansion(PathSafeCrt + ofn.nFileExtension, "crt")) {
					...
					//std::string str_to_write = peep_Сreature->write_myself();
					//
					...
					try {
						fout << peep_Сreature->write_myself();
		...
		case IndexButtonLoadCrt:
		{
			...
			if (GetOpenFileNameA(&ofn)) {
				if (CheckExpansion(PathLoadCrt + ofn.nFileExtension, "crt")) {
					...
					Сreature* load_peep_Сreature = nullptr;
					try {
						load_peep_Сreature = parse_str_to_Creature({ 0, 0 }, str_to_read);

						if (!life_peep_Creature) { delete peep_Сreature; }
						peep_Сreature = load_peep_Сreature;
						load_peep_Сreature = nullptr;

						life_peep_Creature = false;
						last_draw_peep_Creature = true;

						MaxPosSliderPeepBrain = peep_Сreature->get_brain_size() - 1;
						...
						peep_Сreature->build_brain(hWnd);
						peep_Сreature->draw_brain();
						DrawInterface();
					}
	...
	{		
		...
		if (xPos >= margin_x && xPos < (margin_x + size_cell * size_map_x) &&
			...
			if (wParam & MK_SHIFT) {
				...
				if (peep_Сreature != nullptr) {
					place->set_Creature(peep_Сreature->copy(map_cord));
				}
				else {
					place->set_Creature();
				}
				...
				if (!life_peep_Creature) {
					delete peep_Сreature;
					life_peep_Creature = true;
				}
				peep_Сreature = nullptr;
				if (place->get_Type_Creature() != Type_Creature::Void) {

					peep_Сreature = place->get_Creature();
