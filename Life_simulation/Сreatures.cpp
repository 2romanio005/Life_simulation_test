...
bool Ñreature::Ñondition::use(int power_step) {
	...
	if (this->Cond()) {       // ïîôèêñèòü êîãäà ïîñëå óñëîâèÿ ïðèøëî ê äåéñòâèþ è íå äîäåëàåò åãî äî êîíöà òê óæå ïîòðàòèë ñòåïû  !!!!!!!!!!!
...
std::pair<int, int> near_cell_cord(std::pair<int, int> now_map_cord, Direction to_dir)
{
	...
	case Direction::LEFT:
		...
		break;
	case Direction::UNDER:// òàê õîðîøî
		break;
	}
	return now_map_cord;
}
...
Ñreature::Ñreature(std::pair<int, int> map_cord, int energy, int lim_energy, Direction dir, int age, std::vector<Action*>* brain, unsigned int iter)
{
	...
	if (brain == nullptr) {
		...
		for (auto& act : *brain)
		{
			act->set_Creature(this);    // ïîìåòèë êàê ñâîé
		}
		this->brain = *brain;    // ñêîïèðîâàë ìîçã è õðàíèò êàê ñâîé
	}
}
...
void Ñreature::step() {
	if (this->flag_step) {
		...
		for (int i = 0; i < limit_power_step && !this->brain[this->iter]->use(); ++i) {};   // öèêë äëÿ ïðîêðóòêè óñëîâèé 
...
void Ñreature::one_step_finish()
{
	...
	if (this->energy > this->lim_energy) {
		...
		if (near_place->get_Type_Creature() == Type_Creature::Void) {
			...
			if (rand() % 100 < mut_chence) {    // ìóòàöèÿ
		...
		/*Direction rand_dir = Direction((rand() % 4) - 4);     // âàðèàíò ñ ïðîñìîòðîì âàðèàíòîâ ñî âñåõ ñòîðîí
		for (int i = 0; i < 4; i++)
		{
			...
			if (this->see[test_dir][0]->get_Type_Creature() == Type_Creature::Void) {
				...
				std::vector<Creature_Scavenger::Ñondition> br = this->brain;
				int new_lim_energy = this->lim_energy;
